...
void* routine(void *cust_num) {
	...
	while(remaining_N_tel == 0) {
		printf("Customer %d is on hold.\n", *cust_n);
		rc = pthread_cond_wait(&cond_tel, &mutex_tel);
	}
	remaining_N_tel--;
	printf("Customer %d is on line with a telephonist.\n", *cust_n);
	...
	if(zone_n == 0) {
		rc = pthread_mutex_lock(&mutex_search_A);
	} else {
		rc = pthread_mutex_lock(&mutex_search_B);
	}
	...
	if(zone_n == 0) {
		rc = pthread_mutex_unlock(&mutex_search_A);
	} else {
		rc = pthread_mutex_unlock(&mutex_search_B);
	}
	...
	if(seats_row == -1) {	// If avaible seats not found
	...
	while(remaining_N_cash == 0) {
		printf("Customer %d waits for cashier.\n", *cust_n);
...
int main(int argc, char* argv[]) {
	...
	pthread_mutex_init(&mutex_search_A, NULL);
	pthread_mutex_init(&mutex_search_B, NULL);
	...
	pthread_mutex_destroy(&mutex_search_A);
	pthread_mutex_destroy(&mutex_search_B);
